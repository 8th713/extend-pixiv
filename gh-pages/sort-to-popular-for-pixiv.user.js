...
/***/ function(module, exports, __webpack_require__) {
	...
	var MENU_ITEMS = '.column-order-menu > .menu-items';
	var IMAGE_ITEMS = '._image-items';
	var IMAGE_ITEM = '.autopagerize_page_element .image-item';
	var BKMK_COUNT_CLASS = 'bookmark-count';

	function createList() {
	  var count = 0;
	  var pool = _kefir2['default'].pool();
	  var list = pool.map(function (el) {
	    var bkmk = el.getElementsByClassName(BKMK_COUNT_CLASS)[0];
	    var score = +(bkmk && bkmk.textContent || 0);
	    var index = count++;

	    el.score = score;
	    el.index = index;
	    return el;
	  }).scan(function (acc, el) {
	    return acc.concat(el);
	  }, []).debounce(0);

	  list.add = function (el) {
	    pool.plug(_kefir2['default'].constant(el));
	  };
	...
	var listStream = createList();

	(0, _utils.insertButton)('人気順', MENU_ITEMS).map(function (event) {
	  return event.target.classList.contains(_utils.ACTIVE_CLASS);
	}).combine(listStream, function (sorted, list) {
	  if (sorted) {
	    return sortByPopular(list);
	  }
	  return list.slice();
	}).map(_utils.createFragment).onValue(function (fragment) {
	  document.querySelector(IMAGE_ITEMS).appendChild(fragment);
	});

	(0, _utils.getApStream)().map(function (event) {
	  return event.target;
	}).onValue(listStream.add);

	(0, _utils.$$)(IMAGE_ITEM).forEach(listStream.add);
...
/***/ function(module, exports, __webpack_require__) {
	...
	/***/ function(module, exports, __webpack_require__) {
		...
		// (Stream, Stream|Property) -> Stream
		// (Property, Stream|Property) -> Property
		var takeWhileBy = __webpack_require__(76);
		...
		// (Stream, Stream|Property) -> Stream
		// (Property, Stream|Property) -> Property
		var skipWhileBy = __webpack_require__(77);
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.getApStream = getApStream;
	...
	function insertButton(text, selector) {
	  ...
	  return _kefir2['default'].fromEvents(btn, 'click').onValue(function (event) {
	    event.preventDefault();
	    event.target.classList.toggle(ACTIVE_CLASS);
	  });
	}

	function createFragment(list) {
	  var fragment = document.createDocumentFragment();
	  var separators = $$('.autopagerize_page_separator');
	  var infos = $$('.autopagerize_page_info');
	...
	function getApStream() {
	  return _kefir2['default'].merge([_kefir2['default'].fromEvents(document.body, 'AutoPagerize_DOMNodeInserted'), _kefir2['default'].fromEvents(document.body, 'AutoPatchWork.DOMNodeInserted')]);
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.push([module.id, ".popular-introduction {\n  position: static;\n}\n.image-item {\n  height: 249px!important;\n}\n", ""]);
